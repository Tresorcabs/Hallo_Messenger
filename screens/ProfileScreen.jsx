import { View, Text, Button, Image, TouchableOpacity, TextInput, Platform, Keyboard, TouchableWithoutFeedback } from 'react-native';
    ...
    return (
        ...
        <View className="flex-col items-center w-full h-full bg-primary" >
            ...
            <Animated.View entering={FadeInUp.delay(150).duration(1000).springify()} className="flex-row items-center content-center w-full gap-10 h-1/6" style={{ marginVertical: 20, marginTop: 10 }}>
                ...
                <Text className="text-2xl font-bold text-white " >Inscription ( Etape 4 / 5)</Text>
            </Animated.View>
            ...
            <Animated.View entering={FadeInDown.delay(250).duration(5000).springify()} className="flex-col items-center content-center justify-between w-full pt-16 pb-16 bg-white rounded-l-3xl h-5/6" style={{ borderTopLeftRadius: 70, borderTopRightRadius: 70, shadowColor: "#000" }}>
                ...
                <View className="flex-col items-center content-center w-full">
                    ...
                    {/** Profile text */}
                    <Animated.View entering={FadeInUp.delay(300).duration(1000).springify()} className="flex-row items-center content-center justify-center w-full m-5">

                        <Text className="text-lg text-center text-primary">Sélectionnez une photo de profil</Text>
                    </Animated.View>


                    {/** Profile selector  */}
                    <Animated.View entering={FadeInUp.delay(400).duration(1000).springify()} className="items-center content-center w-full ">
                        <View className="p-3 m-5 rounded-full bg-secondary-btn-bg border-primary-200" style={{ borderWidth: 1 }}>
                            {/*placeholder image */}
                            {image ? ( <Image source={{ uri: image }} className="w-40 h-40 rounded-full" /> )
                                : (<Image source={require('../assets/images/placeholder_avatar.png')} className="w-40 h-40 rounded-full" />)
                        }
                            
                        <TouchableOpacity className="absolute bottom-0 p-3 rounded-full right-2 bg-primary" onPress={pickImage}>
                    ...
                    {/** Memo Input */}
                    <Animated.View entering={FadeInUp.delay(400).duration(1000).springify()} className="items-center content-center w-full ">
                        <TextInput className="w-4/5 p-3 m-5 border-primary-200 rounded-xl placeholder:text-behind-input" style={{ borderWidth: 1 }} placeholder="Saisissez votre mémo" multiline={true}></TextInput>
                    </Animated.View>
                </View>
                ...
                <View className="flex items-center content-center justify-center w-full h-1/3">
                    ...
                    <Animated.View entering={FadeInUp.delay(250).duration(1000).springify()} className="items-center justify-center w-full pl-8 pr-8">
                        <TouchableOpacity className="w-4/5 p-3 m-5 bg-primary rounded-xl" onPress={() => navigation.navigate('SignUp5')} >
